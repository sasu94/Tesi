...
module.exports = {
    ...
    newProj: function (name, user, cb) {
        connection.getConnection(function (err, conn) {
            conn.query('insert into Project (Name,User) values (?,?)', [name, user], function (error, results) {
                conn.release();
                cb(results.insertId);
            });
        })
    ...
    removeSubjectsBySample: function (sample,cb) {
        connection.getConnection(function (err, conn) {
            conn.query('delete from subject where Id in (select distinct(Subject) from variation where sample in (?))', [sample], function (error, results) {
                conn.release();
                if (error) throw error;
                cb(results);
            });
